...
--2024/47 desio
SELECT * FROM FTPT_NOTIFICA --METTERE P S FLG_STAT_STMP PER RIGENERARE LA NOTIFICA
WHERE NUM_PR_LIN_CREDITO = 3824
AND (COD_TIPO_RAGG='WCW' AND COD_RAGG=17210)
...
SELECT *                                
       FROM FOPT_PARTITA  A                                    
           ,GDET_CTLTIPAR B                                    --
      WHERE A.COD_SOCRIFERIMENTO = '27'
        AND A.NUM_PR_LIN_CREDITO = '151'
        AND A.COD_RAND_COPP_GEST IN (999999026,999999032,999999033,999999034,999999035,999999069)
        AND (A.DAT_CHIU_DEBI_FCTG IS NULL                      
         OR  A.DAT_CHIU_EMIT      IS NULL)                      
         AND B.COD_TIPO_PRTT      = A.COD_TIPO_PRTT            
         AND B.COD_LING_USER      = '086'   
...
SELECT * FROM GDET_CTLTIPAR
 
UPDATE FOPT_PARTITA  -- mettere la data corrente 
SET DAT_CHIU_EMIT = '2024-05-08'
WHERE COD_RAND_PRTT IN (167483100,648294100,545226100,980496100,452897100,334488100,613399100,645870200,976861200,499542200,
994942200,357313200,419553200,870679200,787644200,470394200,94594200,274375200,290236200,551536200,130736200,
869566200,90427200,713627200,761827200,918857200,25328200,23628200,283828200,360968200,951455100,853979200,
60289200,215900300,759250300,383550300,230850300,804561200,310586000,280667000,581928000,13660100,295522100,
...
-- 2024/133 cambio SAE
UPDATE GAGT_SOGGPERG
SET COD_SETT_BAIT = 430
WHERE COD_SOGGETTO = 100363

INSERT INTO "DSFP01"."GDET_RELSGSET" ("COD_SOCRIFERIMENTO","COD_SPEC_GIUR","COD_SETT_BAIT") VALUES ('01','SNC  ',430);
